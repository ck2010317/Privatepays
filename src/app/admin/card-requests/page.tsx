...
import { CreditCard, Check, X, Loader2, User, DollarSign, Clock } from 'lucide-react';
...
export default function CardRequestsPage() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const res = await fetch(`/api/admin/card-requests?status=${status}`);
      const data = await res.json();

      if (!res.ok) {
        throw new Error(data.error || 'Failed to fetch card requests');
      }

      setRequests(data);
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to fetch card requests');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAction = async (requestId: string, action: 'approve' | 'reject', note?: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/card-requests/${requestId}`, {
        method: 'PATCH',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-white">Card Requests</h1>
          <p className="text-gray-400 mt-1">Review and approve card creation requests</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4">
        {requests.length > 0 ? (
                  ...
                  <div>
                    ...
                    <div className="mt-2">
                      <span className="text-sm text-gray-500">
                        User Balance: ${request.user.balance.toFixed(2)}
                      </span>
                  ...
                  {request.status === 'pending' ? (
                    <>
                      ...
                      <button
                        ...
                        disabled={processingId === request.id || request.user.balance < request.amount}
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p>No {filterStatus === 'all' ? '' : filterStatus} card requests</p>
          </div>
        )}
      </div>
    </div>
